class Assignment < ApplicationRecord
  ...
  before_validation(on: :create) do
    if project.nil? && project_id.nil?
      ...
      errors[:project] << "does not exist" unless Project.where(id: project_id).any?
    end
  end
  ...
  def role_exists?
    if assignable_type == "Role" && !assignable_id.nil?
      errors[:role] << "does not exist" unless Role.where(id: assignable_id).any?
    end
  end

  def location_exists?
    if assignable_type == "Location" && !assignable_id.nil?
      errors[:location] << "does not exist" unless Location.where(id: assignable_id).any?
    end
  end
end
